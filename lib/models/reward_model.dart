...
class RewardModel extends Model {
  ...
  RewardModel.fromJson([Map<String, dynamic>? json]) {
    ...
    requestedAt = requestedAtValue != null && requestedAtValue.isNotEmpty ? DateTime.tryParse(requestedAtValue) : null;

    final reviewedAtValue = stringNullFromJson(payload, "reviewed_at");
    reviewedAt = reviewedAtValue != null && reviewedAtValue.isNotEmpty ? DateTime.tryParse(reviewedAtValue) : null;

    reviewNote = stringNullFromJson(payload, "review_note");

    final rewardAttributes = payload["reward"] is Map<String, dynamic> ? payload["reward"] as Map<String, dynamic> : payload;
    final companyAttributes = payload["company"] is Map<String, dynamic> ? payload["company"] as Map<String, dynamic> : <String, dynamic>{};
    ...
    if (requiredPoints <= 0) requiredPoints = intFromJson(rewardAttributes, "points_required");
    if (requiredPoints <= 0) requiredPoints = intFromJson(rewardAttributes, "points");
    if (requiredPoints <= 0) requiredPoints = intFromJson(payload, "points_required");
    if (requiredPoints <= 0) requiredPoints = intFromJson(payload, "points_cost");
    pointsRequired = requiredPoints;

    final rawPointsCostTotal = intFromJson(payload, "points_cost_total");
    pointsCostTotal = rawPointsCostTotal > 0 ? rawPointsCostTotal : pointsRequired;

    var mediaValue = stringFromJson(rewardAttributes, "media");
    if (mediaValue.isEmpty) mediaValue = stringFromJson(rewardAttributes, "image");
    if (mediaValue.isEmpty) mediaValue = stringFromJson(rewardAttributes, "media_url");
    if (mediaValue.isEmpty) mediaValue = stringFromJson(rewardAttributes, "cover");
    ...
    if (companyNameValue.isEmpty) companyNameValue = stringFromJson(payload, "company_name");
    companyName = companyNameValue;

    var companyLogoValue = companyLogoCandidate;
    if (companyLogoValue.isEmpty) companyLogoValue = stringFromJson(payload, "company_logo");
