...
abstract class Model {
  ...
  num numFromJson(dynamic json, String attribute, {int? fix, double defaultValue = 0.0}) {
    try {
      if (json == null || json[attribute] == null) {
        return defaultValue;
      } else if (fix != null) {
        return num.parse((num.tryParse(json[attribute].toString()) ?? json[attribute]).toStringAsFixed(fix));
      } else {
        return num.parse((num.tryParse(json[attribute].toString()) ?? json[attribute]).toString());
      }
    } catch (e) {
      _loggerDebug.red("there are an error in this key : \"\"$attribute\"\" error: $e");
      // _firebaseCrashlytics.jsonToDartRecordError(e, StackTrace.current, attribute, jsonEncode(json ?? {}), "double_from_json");
